...
# --------------------------------------------------------------------------------
# 2. 데이터 로드 및 전처리 (원본 시트명 'SKBS' 고정 및 멀티 시트 대응)
# --------------------------------------------------------------------------------
...
# --------------------------------------------------------------------------------
# [필수 누락 복구] 🏠 6. 브랜드관 성과 분석 (사용자 요청 로직 및 디자인)
# --------------------------------------------------------------------------------
def render_brand_store_analysis(brand_data, sel_years, df_raw):
    ...
    st.markdown("""<div class="info-box"><b>💡 분석 기준:</b> 브랜드관 페이지 유입 고객의 활동성과 실제 구매 전환(당일 결제)을 분석합니다.</div>""", unsafe_allow_html=True)
    with st.container(border=True):
        c1, c2, c3 = st.columns([1.2, 1, 1.2])
        with c1:
...
# --------------------------------------------------------------------------------
# 5. 메인 탭 구성 (Tab 1~6 전체 유지 및 하단 연동 로직)
# --------------------------------------------------------------------------------
