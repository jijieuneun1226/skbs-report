...
<style>
    ...
    /* 배경색 없는 텍스트 요약 섹션 스타일링 */
    .summary-text-container { margin-bottom: 25px; padding-left: 5px; }
    .summary-text-title { font-weight: 800; font-size: 1.15rem; color: #007bff; margin-bottom: 8px; display: flex; align-items: center; }
    .insight-text-title { font-weight: 800; font-size: 1.15rem; color: #f59e0b; margin-bottom: 8px; display: flex; align-items: center; }
    .custom-list { list-style-type: none; padding-left: 5px; margin: 0; }
    .custom-list li { margin-bottom: 6px; font-size: 14px; color: #1f2937; line-height: 1.5; position: relative; padding-left: 20px; }
    .custom-list li::before { content: "•"; color: #6b7280; font-weight: bold; display: inline-block; width: 1em; margin-left: -1em; }
    .highlight-bold { font-weight: 700; color: #111827; }
</style>
...
with tab2:
    ...
    if not df_final.empty:
        ...
        ranking_raw = df_final.groupby(['사업자번호', '거래처명', '진료과']).agg({'매출액': 'sum'}).sort_values('매출액', ascending=False).head(100).reset_index()
        top100_sum = ranking_raw['매출액'].sum()
        last_p = df_raw.groupby('사업자번호')['매출일자'].max()
        ranking_raw['최근구매일'] = ranking_raw['사업자번호'].map(last_p)
        cur_d = df_raw['매출일자'].max()
        risk_cnt = len(ranking_raw[(cur_d - ranking_raw['최근구매일']).dt.days >= 90])
        cls_data = classify_customers(df_raw, sel_years[0])
        st_counts = cls_data['상태'].value_counts()

        st.markdown(f"""
        <div class="summary-text-container">
            <div class="summary-text-title">📊 데이터 요약</div>
            <ul class="custom-list">
                <li>상위 100대 매출 합계: <span class="highlight-bold">{top100_sum:,.0f} 백만원</span> (전체 매출의 {(top100_sum/total_s*100):.1f}% 차지)</li>
                <li>거래처 상태 분포: <span class="highlight-bold">기존({st_counts.get('✅ 기존 (유지)', 0)}처), 신규({st_counts.get('🆕 신규 (New)', 0)}처), 재유입({status_counts.get('🔄 재유입 (복귀)', 0) if 'status_counts' in locals() else 0}처), 이탈({st_counts.get('📉 1년 이탈', 0)}처)</span></li>
                <li>평균 객단가: 처당 약 <span class="highlight-bold">{(total_s / df_final['사업자번호'].nunique() * 100):,.0f} 만원</span></li>
                <li>최고 매출 거래처: <span class="highlight-bold">{ranking_raw.iloc[0]['거래처명']}</span> ({ranking_raw.iloc[0]['매출액']:,.0f} 백만원)</li>
            </ul>
        </div>
        <div class="summary-text-container">
            <div class="insight-text-title">💡 스마트 인사이트</div>
            <ul class="custom-list">
                <li><b>VIP 이탈 위험 알림:</b> 현재 상위 100대 거래처 중 <span class="highlight-bold">{risk_cnt}처({risk_cnt}%)</span>가 90일 이상 구매가 없는 이탈 위험 상태입니다. 특히 <b>{ranking_raw.iloc[0]['거래처명']}</b>의 공백이 관리 필요 대상입니다.</li>
                <li><b>신규 vs 이탈 밸런스:</b> 금기 신규 유입은 {st_counts.get('🆕 신규 (New)', 0)}처이며, 이탈은 {st_counts.get('📉 1년 이탈', 0)}처입니다. 유입 대비 이탈 관리에 집중해야 합니다.</li>
                <li><b>진료과 집중도 리스크:</b> 매출의 {(df_final.groupby('진료과')['매출액'].sum().max()/total_s*100):.1f}%가 <span class="highlight-bold">{df_final.groupby('진료과')['매출액'].sum().idxmax()}</span>에 편중되어 있어 라인업 확장이 권장됩니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.expander("🥇 매출 상위 거래처 Top 100 리스트", expanded=True):
        ranking_raw['최근구매일_str'] = ranking_raw['최근구매일'].dt.strftime('%Y-%m-%d')
        ranking_raw['상태'] = (cur_d - ranking_raw['최근구매일']).dt.days.apply(lambda x: '🚨 이탈위험' if x >= 90 else '✅ 정상')
        event_vip = st.dataframe(ranking_raw[['상태', '거래처명', '진료과', '매출액', '최근구매일_str']].style.format({'매출액': '{:,.1f} 백만원'})
        ...
        if len(event_vip.selection.rows) > 0:
            sel_v_idx = ranking_raw.index[event_vip.selection.rows[0]]
            st.dataframe(df_raw[df_raw['사업자번호'] == ranking_raw.loc[sel_v_idx, '사업자번호']].groupby('제품명').agg({'매출액': 'sum'}).sort_values('매출액', ascending=False).style.format({'매출액': '{:,.1f} 백만원'}), use_container_width=True)
    st.markdown("---")
...
with tab3:
    ...
    if not wb_v.empty:
        ...
        <div class="summary-text-container">
            <div class="summary-text-title">📊 데이터 요약</div>
            <ul class="custom-list">
                <li>총 재유입 거래처: <span class="highlight-bold">{len(wb_ids_l)} 처</span> (6개월 공백 후 복귀 기준)</li>
                <li>재유입 발생 총 매출: <span class="highlight-bold">{df_wb_l['올해'].sum():,.0f} 백만원</span></li>
                <li>평균 회복률: <span class="highlight-bold">{df_wb_l['회복률'].mean():.1f}%</span> (과거 최고 매출 대비 현재 비율)</li>
                <li>그룹별 분포: 완전 회복({len(df_wb_l[df_wb_l['회복률'] >= 80])}처), 회복 중({len(df_wb_l[(df_wb_l['회복률']<80)&(df_wb_l['회복률']>=20)])}처), 재진입 초기({len(df_wb_l[df_wb_l['회복률'] < 20])}처)</li>
            </ul>
        </div>
        <div class="summary-text-container">
            <div class="insight-text-title">💡 스마트 인사이트</div>
            <ul class="custom-list">
                <li><b>회복 퀄리티 요약:</b> 올해 재유입된 거래처의 평균 회복률은 <span class="highlight-bold">{df_wb_l['회복률'].mean():.1f}%</span>입니다. 완전 회복 그룹이 실적을 견인 중입니다.</li>
                <li><b>복귀 트리거(Trigger) 제품:</b> 이탈 고객들이 복귀 시 가장 먼저 주문하는 제품은 <span class="highlight-bold">{trig_pv}</span>입니다.</li>
                <li><b>재유입 매출 기여도:</b> 전체 매출 중 재유입 거래처가 기여하는 비중은 <span class="highlight-bold">{(df_wb_l['올해'].sum()/df_final['매출액'].sum()*100):.1f}%</span>입니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
with tab4:
    ...
    if not df_final.empty:
        ...
        <div class="summary-text-container">
            <div class="summary-text-title">📊 데이터 요약</div>
            <ul class="custom-list">
                <li>최다 거래 지역: <span class="highlight-bold">{top_c_v['지역']}</span> ({top_c_v['Count']}처)</li>
                <li>최고 매출 지역: <span class="highlight-bold">{top_s_v['지역']}</span> ({top_s_v['Sales']:,.0f} 백만원)</li>
                <li>거래처 변동: 전년 대비 <span class="highlight-bold">{inc_r['지역']} {int(inc_r['Diff_C'])}처 증가 / {dec_r['지역']} {abs(int(dec_r['Diff_C']))}처 감소</span></li>
            </ul>
        </div>
        <div class="summary-text-container">
            <div class="insight-text-title">💡 스마트 인사이트</div>
            <ul class="custom-list">
                <li><b>커버리지 변동:</b> 현재 <b>{inc_r['지역']}</b>은 거래처 수가 증가하며 시장 점유율을 높이고 있습니다. 반면 <b>{dec_r['지역']}</b>은 이탈 방어 전략이 필요합니다.</li>
                <li><b>지역별 핵심 거점 의존도:</b> 특정 지역의 실적이 소수 거점 병원에 편중되어 있는지 확인하여 분산화 전략을 수립해야 합니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
with tab5:
    ...
    if not df_final.empty:
        ...
        <div class="summary-text-container">
            <div class="summary-text-title">📊 데이터 요약</div>
            <ul class="custom-list">
                <li>최다 판매 제품: <span class="highlight-bold">{top_pv_l['제품명']}</span> ({int(top_pv_l['Qty']):,}개 / {top_pv_l['Sales']:,.0f} 백만원)</li>
                <li>최대 매출 제품군: <span class="highlight-bold">{top_cv_l['제품군']}</span> (전체 매출의 {(top_cv_l['매출액']/df_final['매출액'].sum()*100):.1f}% 차지)</li>
                <li>시장 침투율: 전체 거래처 중 <span class="highlight-bold">{(p_v['Count'].sum()/df_final['사업자번호'].nunique()*100):.1f}%</span>가 제품 구매 중</li>
            </ul>
        </div>
        <div class="summary-text-container">
            <div class="insight-text-title">💡 스마트 인사이트</div>
            <ul class="custom-list">
                <li><b>제품별 판매 요약:</b> <b>{top_pv_l['제품명']}</b>이 전반적인 실적을 견인하고 있습니다. 성장세가 둔화된 품목은 성숙기 전략이 필요합니다.</li>
                <li><b>제품군별 기여도:</b> <b>{top_cv_l['제품군']}</b> 제품군의 비중이 높으나, 연관 제품군의 동반 구매를 유도하는 크로스셀링 기회가 존재합니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
