...
# --------------------------------------------------------------------------------
# 2. 데이터 로드 및 전처리 (HTML 파싱 & 강제 다운로드)
# --------------------------------------------------------------------------------
...
def render_smart_overview(df_curr, df_raw):
    ...
    with st.container(border=True):
        ...
        with c1:
            st.metric("💰 총 매출 실적", f"{sales_curr:,.0f} 백만원", f"{sales_pct:+.1f}% (전년동기대비)")
...
def render_regional_deep_dive(df):
    ...
    fig = px.scatter(reg_stats, x='Account_Count', y='Per_Account', size='Total_Sales', color='지역', text='지역',
                     labels={'Account_Count': '거래처 수', 'Per_Account': '평균 객단가(백만원)'})
    ...
    with c1:
        ...
        st.plotly_chart(px.imshow(heatmap_norm, labels=dict(x="제품", y="지역", color="비중(%)"), color_continuous_scale="Blues"), use_container_width=True)
    with c2:
...
def render_product_strategy(df):
    ...
    p_stats = df.groupby('제품명').agg(Total_Sales=('매출액', 'sum'), Account_Count=('사업자번호', 'nunique')).reset_index()
    ...
    # ValueError 방지: size가 0이하인 경우 0.1로 보정
    p_stats['Bubble_Size'] = p_stats['Total_Sales'].apply(lambda x: max(x, 0.1))
    
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("#### 1️⃣ 제품 포트폴리오 (BCG Matrix)")
        fig = px.scatter(p_stats, x='Growth_Rate', y='Total_Sales', size='Bubble_Size', color='제품명', text='제품명',
                         labels={'Growth_Rate': '성장률(%)', 'Total_Sales': '매출(백만원)'})
        st.plotly_chart(fig, use_container_width=True)
    with c2:
...
with tab1:
    ...
    c1.metric("선택기간 매출", f"{df_final['매출액'].sum():,.0f} 백만원")
    ...
    with col_b:
        ...
        fig_dual.update_layout(yaxis=dict(title='매출(백만원)'), yaxis2=dict(title='구매처수(처)', overlaying='y', side='right'))
...
with tab4:
    ...
    st.plotly_chart(px.bar(reg_s, x='지역', y='매출액', title="지역별 매출 (백만원)"), use_container_width=True)

with tab5:
    render_product_strategy(df_final)
    st.markdown("---")
    p_main = df_final.groupby('제품명').agg({'매출액': 'sum', '사업자번호': 'nunique'}).reset_index().sort_values('매출액', ascending=False)
