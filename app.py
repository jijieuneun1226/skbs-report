...
# 4. 메인 대시보드 복구
# --------------------------------------------------------------------------------
tabs = st.tabs(["📊 Overview", "🏆 VIP & 이탈", "🔄 재유입 분석", "🗺️ 지역 분석", "📦 제품 분석"])

with tabs[0]:
    st.subheader("📈 성과 요약 (단위: 백만원)")
    c1, c2 = st.columns(2)
    c1.metric("필터 조건 매출액", f"{df_final['매출액_백만'].sum():,.1f} M")
    c2.metric("필터 조건 구매처수", f"{df_final['사업자번호'].nunique():,} 처")
    
    monthly = df_final.groupby('년월').agg({'매출액_백만': 'sum', '사업자번호': 'nunique'}).reset_index()
    fig = go.Figure()
    fig.add_trace(go.Bar(x=monthly['년월'], y=monthly['매출액_백만'], name='매출(M)', yaxis='y1'))
    fig.add_trace(go.Scatter(x=monthly['년월'], y=monthly['사업자번호'], name='처수', yaxis='y2', line=dict(color='red')))
    fig.update_layout(yaxis2=dict(overlaying='y', side='right'), title="월별 매출 및 처수 추이")
    st.plotly_chart(fig, use_container_width=True)

with tabs[1]:
    st.subheader("🏆 VIP 및 이탈 관리")
    st.info("💡 행 선택 시 하단에 상세 품목이 합산되어 표시됩니다.")
    ...
    sel_vip = st.dataframe(ranking[['상태', '거래처명', '진료과', '매출액_백만', '최근구매일']], use_container_width=True, on_select="rerun", selection_mode="single-row")
    if len(sel_vip.selection.rows) > 0:
        bz_no = ranking.iloc[sel_vip.selection.rows[0]]['사업자번호']
        st.write(f"🔍 **{ranking.iloc[sel_vip.selection.rows[0]]['거래처명']}** 상세 실적")
        st.dataframe(df_raw[df_raw['사업자번호'] == bz_no].groupby('제품명').agg({'매출액_백만': 'sum', '수량': 'sum'}).reset_index(), use_container_width=True)

    st.markdown("---")
    # 상태별 그룹 분석 복구
    cls_df = classify_customers(df_raw, sel_years[0])
    sel_st = st.selectbox("👇 분석할 그룹 선택", sorted(cls_df['상태'].unique()))
    st.write(f"📊 **{sel_st}** 그룹 상세 리스트")
    st.dataframe(cls_df[cls_df['상태'] == sel_st].sort_values('해당년도_매출', ascending=False), use_container_width=True)

with tabs[2]:
    st.subheader("🔄 재유입 패턴 분석")
    st.write("💡 제품 선택 시 해당 제품으로 복귀한 고객들의 이탈 전 구매 패턴이 나타납니다.")
    ...
    res_sum = res.groupby('제품명').agg({'사업자번호': 'nunique', '매출액_백만': 'sum'}).reset_index().sort_values('사업자번호', ascending=False)
    ev_res = st.dataframe(res_sum.rename(columns={'사업자번호': '재유입처수'}), use_container_width=True, on_select="rerun", selection_mode="single-row")
    
    if len(ev_res.selection.rows) > 0:
        p_name = res_sum.iloc[ev_res.selection.rows[0]]['제품명']
        st.write(f"🎯 **{p_name}** 복귀 고객의 이탈 전 사용 제품 TOP 10")
...
with tabs[3]:
    ...
    st.plotly_chart(px.scatter(reg_data, x='사업자번호', y='매출액_백만', text='지역', size='매출액_백만', title="지역별 매출/처수 분포"), use_container_width=True)
    
    sel_reg = st.selectbox("🔎 지역 상세 분석 선택", reg_data['지역'].unique())
    if sel_reg:
        st.write(f"🏠 **{sel_reg}** 지역 상위 매출 거래처")
        st.dataframe(df_final[df_final['지역'] == sel_reg].groupby('거래처명').agg({'매출액_백만': 'sum', '수량': 'sum'}).reset_index().sort_values('매출액_백만', ascending=False).head(30), use_container_width=True)

with tabs[4]:
    st.subheader("📦 제품별 판매 현황")
    prod_data = df_final.groupby('제품명').agg({'매출액_백만': 'sum', '수량': 'sum', '사업자번호': 'nunique'}).reset_index().sort_values('매출액_백만', ascending=False)
    ev_p = st.dataframe(prod_data.rename(columns={'사업자번호': '구매처수'}), use_container_width=True, on_select="rerun", selection_mode="single-row")
