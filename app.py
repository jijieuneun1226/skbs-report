...
with tab2:
    if not df_final.empty:
        ...
        st.write(f"• 최고 매출 거래처: **{top100.iloc[0]['거래처명']}**")
        
        st.subheader("💡 스마트 인사이트")
        if risk_cnt > 20:
            st.warning(f"현재 상위 100대 거래처 중 **{risk_cnt}처({risk_cnt}%)**가 90일 이상 구매가 없는 **이탈 위험** 상태입니다.")
        else:
            st.info(f"상위 100대 거래처 중 이탈 위험 업체는 **{risk_cnt}처**로 비교적 안정적입니다.")
    ...
    event_v = st.dataframe(top100[['상태', '거래처명', '진료과', '매출액', '최근구매일_str']], use_container_width=True, on_select="rerun", selection_mode="single-row", height=350)
    if len(event_v.selection.rows) > 0:
        sel_biz = top100.iloc[event_v.selection.rows[0]]['사업자번호']
        st.dataframe(df_raw[df_raw['사업자번호'] == sel_biz].groupby('제품명').agg({'매출액':'sum'}).sort_values('매출액', ascending=False).style.format({'매출액':'{:,.1f}'}), use_container_width=True)
...
with tab3:
    ...
    if not wb_base.empty:
        ...
        st.write(f"• 재유입 매출: **{df_wb_sum['올해'].sum():,.0f} 백만원** / 평균 회복률: **{df_wb_sum['회복률'].mean():.1f}%**")
...
with tab5:
    ...
    if not p_v.empty:
        st.subheader("📊 데이터 요약")
        st.write(f"• 최다 판매 제품: **{p_v.iloc[0]['제품명']}** / 최대 제품군: **{cat_v.iloc[0]['제품군']}**")
