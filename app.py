...
<style>
    ...
    /* 요약 박스 스타일 */
    .summary-container { background-color: #f1f3f6; padding: 15px; border-radius: 10px; margin-bottom: 20px; border: 1px solid #d1d5db; }
    .summary-title { font-weight: 800; font-size: 1.1rem; color: #1f2937; margin-bottom: 10px; display: flex; align-items: center; }
</style>
...
with tab2:
    ...
    if not df_final.empty:
        total_sales_val = df_final['매출액'].sum()
        ranking_all = df_final.groupby(['사업자번호', '거래처명', '진료과']).agg({'매출액': 'sum'}).reset_index()
        top100_all = ranking_all.sort_values('매출액', ascending=False).head(100)
        top100_sum = top100_all['매출액'].sum()
        top100_ratio = (top100_sum / total_sales_val * 100)
        
        cls_data = classify_customers(df_raw, sel_years[0])
        st_counts = cls_data['상태'].value_counts()
        
        last_p_map = df_raw.groupby('사업자번호')['매출일자'].max()
        top100_all['최근구매일'] = top100_all['사업자번호'].map(last_p_map)
        cur_d = df_raw['매출일자'].max()
        top100_all['공백'] = (cur_d - top100_all['최근구매일']).dt.days
        risk_vips = top100_all[top100_all['공백'] >= 90]
        risk_cnt = len(risk_vips)
        top_v = top100_all.iloc[0]

        # [📊 데이터 요약]
        st.markdown(f"""
        <div class="summary-container">
            <div class="summary-title">📊 데이터 요약</div>
            <ul>
                <li><b>상위 100대 매출 합계:</b> {top100_sum:,.0f} 백만원 (전체 매출의 {top100_ratio:.1f}% 차지)</li>
                ...
                <li><b>평균 객단가:</b> 처당 약 {(total_sales_val / df_final['사업자번호'].nunique() * 100):,.0f} 만원</li>
                <li><b>최고 매출 거래처:</b> {top_v['거래처명']} ({top_v['매출액']:,.0f} 백만원)</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)

        # [💡 스마트 인사이트] 박스 색상 가변형
        insight_color = "red" if risk_cnt > 30 else ("orange" if risk_cnt > 10 else "#007bff")
        st.markdown(f"""
        <div style="background-color: #fff9e6; padding: 15px; border-radius: 10px; border-left: 5px solid {insight_color}; margin-bottom: 20px;">
            <div class="summary-title">💡 스마트 인사이트</div>
            <ul>
                <li><b>VIP 이탈 위험 알림:</b> 현재 상위 100대 거래처 중 <b>{risk_cnt}처({risk_cnt}%)</b>가 90일 이상 구매가 없는 이탈 위험 상태입니다. 특히 <b>{top_v['거래처명']}</b>의 공백이 {top_v['공백']}일째 지속되고 있어 즉각적인 관리가 필요합니다.</li>
                <li><b>신규 vs 이탈 밸런스:</b> 금기 신규 유입 거래처는 {st_counts.get('🆕 신규 (New)', 0)}처인 반면, 이탈(1년 기준) 거래처는 {st_counts.get('📉 1년 이탈', 0)}처입니다. 유입 대비 이탈이 많으므로 기존 고객 유지를 위한 프로모션 강화가 우선시됩니다.</li>
                <li><b>진료과 집중도 리스크:</b> 현재 매출의 {(df_final.groupby('진료과')['매출액'].sum().max()/total_sales_val*100):.1f}%가 <b>{df_final.groupby('진료과')['매출액'].sum().idxmax()}</b>에 편중되어 있습니다. 해당 과의 정책 변화에 대비해 타 진료과로의 확장이 권장됩니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.expander("🥇 매출 상위 거래처 Top 100", expanded=True):
        ...
        top100_all['최근구매일'] = top100_all['최근구매일'].dt.strftime('%Y-%m-%d')
        top100_all['상태'] = top100_all['공백'].apply(lambda x: '🚨 이탈위험' if x >= 90 else '✅ 정상')
        event_vip = st.dataframe(top100_all[['상태', '거래처명', '진료과', '매출액', '최근구매일']].style.format({'매출액': '{:,.1f} 백만원'})
        ...
        if len(event_vip.selection.rows) > 0:
            v_idx = top100_all.index[event_vip.selection.rows[0]]
            v_detail = df_raw[df_raw['사업자번호'] == top100_all.loc[v_idx, '사업자번호']].groupby('제품명').agg({'매출액': 'sum'}).reset_index()
...
with tab3:
    ...
    wb_base = df_h[(df_h['사업자번호'].isin(df_final['사업자번호'])) & (df_h['구매간격'] >= 180)].copy()
    
    if not wb_base.empty:
        wb_ids_v = wb_base['사업자번호'].unique()
        s_curr = df_final[df_final['사업자번호'].isin(wb_ids_v)].groupby('사업자번호')['매출액'].sum()
        s_max = df_raw[df_raw['사업자번호'].isin(wb_ids_v)].groupby('사업자번호')['매출액'].max()
        df_wb_v = pd.DataFrame({'올해': s_curr, '최고': s_max}).fillna(0)
        df_wb_v['회복률'] = (df_wb_v['올해'] / df_wb_v['최고'] * 100).replace([np.inf, -np.inf], 0)
        
        full_v = len(df_wb_v[df_wb_v['회복률'] >= 80])
        mid_v = len(df_wb_v[(df_wb_v['회복률'] < 80) & (df_wb_v['회복률'] >= 20)])
        low_v = len(df_wb_v[df_wb_v['회복률'] < 20])
        ...
        <div class="summary-container">
            <div class="summary-title">📊 데이터 요약</div>
            <ul>
                <li><b>총 재유입 거래처:</b> {len(wb_ids_v)} 처 (6개월 공백 후 복귀 기준)</li>
                <li><b>재유입 발생 총 매출:</b> {df_wb_v['올해'].sum():,.0f} 백만원</li>
                <li><b>평균 회복률:</b> {df_wb_v['회복률'].mean():.1f}% (과거 최고 매출 대비 현재 비율)</li>
                <li><b>그룹별 분포:</b> 완전 회복({full_v}처), 회복 중({mid_v}처), 재진입 초기({low_v}처)</li>
            </ul>
        </div>
        <div class="insight-box">
            <div class="summary-title">💡 스마트 인사이트</div>
            <ul>
                <li><b>회복 퀄리티 요약:</b> 올해 재유입된 거래처의 평균 회복률은 <b>{df_wb_v['회복률'].mean():.1f}%</b>입니다. 완전 회복 그룹이 {full_v}처로, 복귀 고객들의 충성도가 정상화되고 있습니다.</li>
                <li><b>복귀 트리거(Trigger) 제품:</b> 이탈 고객들이 복귀 시 가장 먼저 주문하는 제품은 <b>{trig_p}</b>입니다. 마케팅 시 이 제품을 전면에 내세우는 것이 효과적입니다.</li>
                <li><b>재유입 매출 기여도:</b> 전체 매출 중 재유입 거래처가 기여하는 비중은 <b>{(df_wb_v['올해'].sum()/df_final['매출액'].sum()*100):.1f}%</b>입니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
with tab4:
    # 탭 4 데이터 요약 및 인사이트
    if not df_final.empty:
        reg_s_v = df_final.groupby('지역').agg(Sales=('매출액', 'sum'), Count=('사업자번호', 'nunique')).reset_index()
        top_c_reg = reg_s_v.loc[reg_s_v['Count'].idxmax()]
        top_s_reg = reg_s_v.loc[reg_s_v['Sales'].idxmax()]
        
        curr_y_v = df_final['년'].max()
        df_ly = df_raw[(df_raw['년'] == (curr_y_v-1)) & (df_raw['월'].isin(df_final['월'].unique()))]
        ly_stats = df_ly.groupby('지역').agg(LY_Count=('사업자번호', 'nunique'), LY_Sales=('매출액', 'sum')).reset_index()
        reg_comp = reg_s_v.merge(ly_stats, on='지역', how='left').fillna(0)
        reg_comp['Diff_C'] = reg_comp['Count'] - reg_comp['LY_Count']
        inc_reg = reg_comp.loc[reg_comp['Diff_C'].idxmax()]
        dec_reg = reg_comp.loc[reg_comp['Diff_C'].idxmin()]
        
        st.markdown(f"""
        <div class="summary-container">
            <div class="summary-title">📊 데이터 요약</div>
            <ul>
                <li><b>최다 거래 지역:</b> {top_c_reg['지역']} ({top_c_reg['Count']}처)</li>
                <li><b>최고 매출 지역:</b> {top_s_reg['지역']} ({top_s_reg['Sales']:,.0f} 백만원)</li>
                <li><b>거래처 변동:</b> 전년 대비 {inc_reg['지역']} {int(inc_reg['Diff_C'])}처 증가 / {dec_reg['지역']} {abs(int(dec_reg['Diff_C']))}처 감소</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class="insight-box">
            <div class="summary-title">💡 스마트 인사이트</div>
            <ul>
                <li><b>커버리지 변동:</b> 현재 <b>{inc_reg['지역']}</b>은 거래처 수가 전년 대비 증가하며 확장을 보이고 있습니다. 반면 <b>{dec_reg['지역']}</b>은 감소세로 영업 강화가 필요합니다.</li>
                <li><b>지역별 핵심 거점 의존도:</b> <b>{df_risk_v.iloc[0]['지역']}</b>은 상위 1개 병원의 매출 비중이 <b>{df_risk_v.iloc[0]['의존도']:.1f}%</b>에 달합니다. 거점 병원 이탈 리스크 관리가 시급합니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
...
with tab5:
    # 탭 5 데이터 요약 및 인사이트
    if not df_final.empty:
        p_main_v = df_final.groupby('제품명').agg(Sales=('매출액', 'sum'), Qty=('수량', 'sum'), Count=('사업자번호', 'nunique')).reset_index().sort_values('Sales', ascending=False)
        top_pv = p_main_v.iloc[0]
        cat_s_v = df_final.groupby('제품군')['매출액'].sum().reset_index()
        top_cat_v = cat_s_v.loc[cat_s_v['매출액'].idxmax()]
        penetration_v = (p_main_v['Count'].sum() / df_final['사업자번호'].nunique() * 100)

        st.markdown(f"""
        <div class="summary-container">
            <div class="summary-title">📊 데이터 요약</div>
            <ul>
                <li><b>최다 판매 제품:</b> {top_pv['제품명']} ({int(top_pv['Qty']):,}개 / {top_pv['Sales']:,.0f} 백만원)</li>
                <li><b>최대 매출 제품군:</b> {top_cat_v['제품군']} (전체 매출의 {(top_cat_v['매출액']/df_final['매출액'].sum()*100):.1f}% 차지)</li>
                <li><b>시장 침투율:</b> 전체 거래처 중 {penetration_v:.1f}%가 최소 1개 이상의 제품을 구매 중</li>
            </ul>
        </div>
        <div class="insight-box">
            <div class="summary-title">💡 스마트 인사이트</div>
            <ul>
                <li><b>제품별 판매 요약:</b> <b>{top_pv['제품명']}</b>이 전체 실적을 견인하고 있습니다. 다만 점유율 대비 성장세가 둔화된 품목은 신규 라인업 교체가 필요합니다.</li>
                <li><b>제품군별 기여도:</b> <b>{top_cat_v['제품군']}</b> 제품군이 핵심 축이나, 미래 동력을 위해 타 제품군의 병원 채택률을 높여야 합니다.</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown('<p class="guide-text">💡 체크박스 클릭 시 상세 리스트 표시</p>', unsafe_allow_html=True)
