...
def render_product_strategy(df):
    if df.empty: return
    st.markdown("### 💊 제품별 전략 BCG")
    p_stats = df.groupby('제품명').agg(Sales=('매출액', 'sum'), Count=('사업자번호', 'nunique')).reset_index()
    fig = px.scatter(p_stats, x='Count', y='Sales', size='Sales', color='제품명', text='제품명')
    st.plotly_chart(fig, use_container_width=True)
...
with tab2:
    if not df_final.empty:
        ...
        st.subheader("📊 데이터 요약")
        st.write(f"• 상위 100대 매출 합계: **{top100['매출액'].sum():,.0f} 백만원** ({(top100['매출액'].sum()/total_s*100):.1f}%)")
        ...
        st.write(f"• **VIP 이탈 위험:** 상위 100대 중 **{risk_cnt}처({risk_cnt}%)**가 90일 이상 공백 상태입니다.")
        st.write(f"• **진료과 집중도:** 매출의 {(df_final.groupby('진료과')['매출액'].sum().max()/total_s*100):.1f}%가 **{df_final.groupby('진료과')['매출액'].sum().idxmax()}**에 편중되어 있습니다.")
...
with tab3:
    ...
    if not wb_v_sum.empty:
        wb_ids_v = wb_v_sum['사업자번호'].unique()
        ...
        st.subheader("📊 데이터 요약")
        st.write(f"• 총 재유입 거래처: **{len(wb_ids_v)} 처** (6개월 공백 기준) / 평균 회복률: **{df_wb_f['회복률'].mean():.1f}%**")
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **복귀 트리거:** 이탈 고객 복귀 시 가장 먼저 찾는 제품은 **{wb_v_sum.groupby('제품명').size().idxmax()}**입니다.")
...
with tab4:
    ...
    if not reg_v.empty:
        st.subheader("📊 데이터 요약")
        st.write(f"• 최고 매출 지역: **{reg_v.iloc[0]['지역']}** ({reg_v.iloc[0]['Sales']:,.0f}M)")
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **커버리지:** {reg_v.sort_values('Count', ascending=False).iloc[0]['지역']} 지역이 가장 활발한 거래를 보이고 있습니다.")
    st.markdown("### 🗺️ 지역별 상세 실적")
    st.markdown('<p class="guide-text">💡 지역 선택 시 상세 리스트가 표시됩니다.</p>', unsafe_allow_html=True)
...
with tab5:
    ...
    if not p_v.empty:
        ...
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **성장 견인:** {p_v.iloc[0]['제품명']}이 실적의 핵심이며, 크로스셀링 기회 분석을 권장합니다.")
