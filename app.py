...
if is_edit_mode:
    with st.sidebar:
        ...
        if st.button("🔗 축약 공유 링크 생성"):
            ...
            st.success("축약 공유 링크가 생성되었습니다!"); st.code(base_url + p_string, language="text")
...
with tab2:
    if not df_final.empty:
        ...
        last_p = df_raw.groupby('사업자번호')['매출일자'].max(); ranking_v['최근구매일'] = ranking_v['사업자번호'].map(last_p)
        cur_d = df_raw['매출일자'].max(); risk_cnt = len(ranking_v[(cur_d - ranking_v['최근구매일']).dt.days >= 90])

        # [수정사항 2 반영] 글자색 해결: 테마에 자동 최적화되는 subheader와 write 사용
        st.subheader("📊 데이터 요약")
        st.write(f"• 상위 100대 매출 합계: **{t100_sum:,.0f} 백만원** (전체 매출의 {(t100_sum/total_s*100):.1f}% 차지)")
        st.write(f"• 거래처 상태 분포: **기존({st_c.get('✅ 기존 (유지)',0)}처), 신규({st_c.get('🆕 신규 (New)',0)}처), 재유입({st_c.get('🔄 재유입 (복귀)',0)}처), 이탈({st_c.get('📉 1년 이탈',0)}처)**")
        st.write(f"• 최고 매출 거래처: **{ranking_v.iloc[0]['거래처명']}** ({ranking_v.iloc[0]['매출액']:,.0f} 백만원)")

        st.subheader("💡 스마트 인사이트")
        st.write(f"• **VIP 이탈 위험 알림:** 상위 100대 거래처 중 **{risk_cnt}처({risk_cnt}%)**가 90일 이상 구매가 없는 이탈 위험 상태입니다.")
    ...
    with st.expander("🥇 매출 상위 거래처 Top 100", expanded=True):
        ...
        if len(event_vip.selection.rows) > 0:
            sel_biz = ranking_v.iloc[event_vip.selection.rows[0]]['사업자번호']
...
with tab3:
    ...
    if not wb_v.empty:
        ...
        # [수정사항 2 반영] 글자색 해결
        st.subheader("📊 데이터 요약")
        st.write(f"• 총 재유입 거래처: **{len(wb_ids_l)} 처** (6개월 공백 후 복귀 기준) / 평균 회복률: **{df_wb_l['회복률'].mean():.1f}%**")
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **복귀 트리거 제품:** 이탈 고객들이 복귀 시 가장 먼저 찾는 제품은 **{wb_v.groupby('제품명').size().idxmax()}**입니다.")
...
with tab4:
    ...
    if not reg_v.empty:
        ...
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **커버리지 분석:** 현재 **{reg_v.sort_values('Count', ascending=False).iloc[0]['지역']}** 지역이 활발한 거래를 보이고 있습니다.")

    render_regional_deep_dive(df_final)
    st.markdown("---")
    st.markdown("### 🗺️ 지역별 상세 리스트")
    sel_r_v = st.selectbox("🔎 분석할 지역 선택", reg_v['지역'].unique(), key="p4_reg_v")
    ...
    with col_r2: st.plotly_chart(px.pie(df_final[df_final['지역'] == sel_r_v], values='매출액', names='제품명', hole=0.3, title=f"[{sel_r_v}] 제품 비중"), use_container_width=True)
    st.dataframe(df_final[df_final['지역'] == sel_r_v].groupby(['거래처명', '제품명']).agg({'매출액': 'sum', '수량': 'sum'}).reset_index().sort_values('매출액', ascending=False).head(50), use_container_width=True)
...
with tab5:
    ...
    if not p_v.empty:
        # [수정사항 2 반영] 글자색 해결
        st.subheader("📊 데이터 요약")
        st.write(f"• 최다 판매 제품: **{p_v.iloc[0]['제품명']}** ({int(p_v.iloc[0]['Qty']):,}개) / 최대 제품군: **{cat_v.iloc[0]['제품군']}**")
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **성장 동력 분석:** {p_v.iloc[0]['제품명']}이 실적 성장을 주도하고 있습니다.")
    ...
    ev_p_v = st.dataframe(p_v.rename(columns={'Count':'구매처수'}), use_container_width=True, on_select="rerun", selection_mode="single-row", height=300)
    if len(ev_p_v.selection.rows) > 0:
        sel_p_v = p_v.iloc[ev_p_v.selection.rows[0]]['제품명']
        st.dataframe(df_final[df_final['제품명'] == sel_p_v].groupby('거래처명').agg({'매출액': 'sum'}).reset_index().sort_values('매출액', ascending=False), use_container_width=True)
