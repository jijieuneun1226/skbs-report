...
def render_product_strategy(df):
    ...
    with c1:
        ...
        st.plotly_chart(px.scatter(p_stats, x='Growth_Rate', y='Total_Sales', size='Bubble_Size', color='제품명', text='제품명', labels={'Growth_Rate': '성장률(%)', 'Total_Sales': '매출(백만원)'}), use_container_width=True)
    with c2:
...
with tab1:
    ...
    c1, c2 = st.columns(2)
    c1.metric("선택기간 총 매출", f"{df_final['매출액'].sum():,.0f} 백만원")
...
with tab4:
    ...
    st.plotly_chart(px.bar(reg_s, x='지역', y='매출액', title="지역별 총 매출(백만원)"), use_container_width=True)
