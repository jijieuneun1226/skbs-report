...
# [수정 1] URL 길이를 획기적으로 줄이기 위해 'all' 키워드를 인식하는 파라미터 로드 함수
...
def render_smart_overview(df_curr, df_raw):
    ...
    st.markdown(f"### 🚀 {current_year}년 Summary")
    ...
    with col_a: st.plotly_chart(px.pie(df_curr, values='매출액', names='진료과', hole=0.4, title="진료과 비중"), use_container_width=True)
    with col_b: st.plotly_chart(px.bar(df_curr.groupby('년월')['매출액'].sum().reset_index(), x='년월', y='매출액', text_auto='.1f', title="월별 매출 추이"), use_container_width=True)
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    with col_a: st.plotly_chart(px.scatter(df_wb, x='최고', y='올해', color='상태', hover_name='거래처명', size='bubble', color_discrete_map={"완전 회복":"green","회복 중":"orange","재진입 초기":"red"}), use_container_width=True)
    with col_b: event_wb = st.dataframe(df_wb[['상태', '거래처명', '올해', '회복률']], use_container_width=True, on_select="rerun", selection_mode="single-row")
...
def render_product_strategy(df):
    ...
    with st.expander("🧩 함께 팔기(Cross-selling) 기회 분석기", expanded=True):
        ...
        if b_p != t_p:
            ...
            if targets:
                t_info = df[(df['거래처명'].isin(targets)) & (df['제품명']==b_p)].groupby(['거래처명','지역'])['매출액'].sum().reset_index().sort_values('매출액', ascending=False).head(50)
                st.dataframe(t_info, use_container_width=True)
...
with tab2:
    if not df_final.empty:
        ...
        # [수정 2 반영] 글자색 해결: st.subheader와 st.write 표준 방식 사용 (시스템 테마 대응)
        st.subheader("📊 데이터 요약")
        st.write(f"• 상위 100대 매출 합계: **{top100['매출액'].sum()/100:,.1f}억원** (전체 매출의 **{(top100['매출액'].sum()/total_s*100):.1f}%** 차지)")
        ...
        st.write(f"• 최고 매출 거래처: **{top100.iloc[0]['거래처명']}** ({top100.iloc[0]['매출액']:,.1f} 백만원)")
        
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **VIP 이탈 위험 알림:** 현재 상위 100대 거래처 중 **{risk_cnt}처({risk_cnt}%)**가 90일 이상 구매가 없는 이탈 위험 상태입니다. 특히 **{top100.iloc[0]['거래처명']}**의 공백 관리가 필요합니다.")
...
with tab3:
    ...
    if not wb_v_sum.empty:
        ...
        f_rec = len(df_wb_f[df_wb_f['회복률']>=80]); m_rec = len(df_wb_f[(df_wb_f['회복률']<80)&(df_wb_f['회복률']>=20)]); l_rec = len(df_wb_f[df_wb_f['회복률']<20])
        trig_p = wb_v_sum.groupby('제품명').size().idxmax()
        
        st.subheader("📊 데이터 요약")
        st.write(f"• 총 재유입 거래처: **{len(wb_ids)}처** (6개월 공백 후 복귀 기준) / 재유입 매출: **{df_wb_f['올해'].sum():,.0f} 백만원**")
        st.write(f"• 평균 회복률: **{df_wb_f['회복률'].mean():.1f}%** / 분포: 완전({f_rec}), 회복 중({m_rec}), 초기({l_rec})")
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **회복 퀄리티 요약:** 올해 재유입 고객의 평균 회복률은 **{df_wb_f['회복률'].mean():.1f}%**입니다. '완전 회복' 그룹이 **{f_rec}처**로 충성도가 빠르게 정상화되고 있습니다.")
        st.write(f"• **복귀 트리거 제품:** 이탈 고객들이 복귀 시 가장 먼저 찾는 제품은 **{trig_p}**입니다.")
...
with tab4:
    ...
    df_ly_r = df_raw[(df_raw['년'] == y_v-1) & (df_raw['월'].isin(m_v))].groupby('지역').agg(LY_C=('사업자번호','nunique'), LY_S=('매출액','sum')).reset_index()
    reg_m = reg_v.merge(df_ly_r, on='지역', how='left').fillna(0)
    inc_r = reg_m.loc[(reg_m['Count']-reg_m['LY_C']).idxmax()]; dec_r = reg_m.loc[(reg_m['Count']-reg_m['LY_C']).idxmin()]
    
    st.subheader("📊 데이터 요약")
    if not reg_v.empty:
        st.write(f"• 최다 거래 지역: **{reg_v.sort_values('Count', ascending=False).iloc[0]['지역']}** ({int(reg_v.sort_values('Count', ascending=False).iloc[0]['Count'])}처)")
        ...
        st.subheader("💡 스마트 인사이트")
        st.write(f"• **커버리지 변동:** 현재 **{inc_r['지역']}**은 전년비 거래처가 크게 증가하며 확장을 보이고 있습니다.")
        st.write(f"• **지역별 거점 의존도:** **{reg_v.iloc[0]['지역']}** 지역은 거점 병원 매출 비중이 높아 분산 전략이 권장됩니다.")
...
with tab5:
    ...
    ev_p = st.dataframe(p_v[['제품명','Qty','Sales','Count']].rename(columns={'Count':'구매처수'}), use_container_width=True, on_select="rerun", selection_mode="single-row", height=300)
