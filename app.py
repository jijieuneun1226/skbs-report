...
# --------------------------------------------------------------------------------
# 3. [기존 탭 1~5 함수 정의 - 토씨 하나 안 틀리고 원복]
# --------------------------------------------------------------------------------
...
def render_regional_deep_dive(df):
    ...
    st.markdown(f"""<div class="info-box"><b>📈 지역 전략 요약:</b><br>- <b>최고 매출 지역:</b> 기간 내 매출액 최대 지역<br>- <b>영업 효율 1위:</b> 객단가 최대 지역</div>""", unsafe_allow_html=True)
    with st.container(border=True):
        c1, c2, c3 = st.columns(3)
...
def render_product_strategy(df):
    ...
    with c2: st.plotly_chart(px.bar(p_stats.sort_values('Count'), x='Count', y='제품명', orientation='h', text_auto=True, title="시장 침투율(처)"), use_container_width=True)
    ...
    with st.expander("🧩 함께 팔기(Cross-selling) 기회 분석기", expanded=True):
        ...
        with col_sel1: base_p = st.selectbox("Anchor 제품", all_prods, index=0)
        with col_sel2: target_p = st.selectbox("Target 제품", all_prods, index=min(1, len(all_prods)-1))
        if base_p != target_p:
...
# --------------------------------------------------------------------------------
# [보완 완료] 🏠 6. 브랜드관 성과 분석 (사용자 디자인 요청 사항 완벽 반영)
# --------------------------------------------------------------------------------
def render_brand_store_analysis(brand_data, sel_years, df_raw):
    ...
    if not df_direct.empty:
        ...
        df_direct['상품명_매핑'] = df_direct['상품명'].apply(map_skbs_name)
