...
def load_data_from_drive(file_id):
    ...
    try:
        ...
        # [수정] 여러 시트를 읽기 위해 ExcelFile 사용 및 시트명 'SKBS' 반영
...
def render_brand_store_analysis(brand_data):
    ...
    st.plotly_chart(px.line(brand_data['Brand_Monthly'], x='월', y=['UV', 'PV'], markers=True, 
                          color_discrete_map={'UV': '#4e79a7', 'PV': '#e15759'},
                          title="월별 방문자(UV) 및 페이지뷰(PV) 추이"), use_container_width=True)
    ...
    with col_right:
        ...
        if not comp_df.empty:
            st.plotly_chart(px.pie(comp_df, values='매출', names='상품명', hole=0.4, 
                             title="브랜드관 방문 고객이 선택한 타 브랜드 품목"), use_container_width=True)
