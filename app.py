...
def load_data_from_drive(file_id):
    ...
    col_map = {'매출일자':['매출일자','날짜','Date'],'제품명':['제품명변환','제품명'],'합계금액':['합계금액','공급가액','매출액'],'수량':['수량','Qty'],'사업자번호':['사업자번호','BizNo'],'거래처명':['거래처명','병원명'],'진료과':['진료과'],'제품군':['제품군'],'지역':['지역']}
...
# (분석 함수들은 기존 로직과 동일하여 지면상 생략하거나 내부 배치)
...
def render_winback_quality(df_final, df_raw, current_year):
    ...
    s_curr = df_final[df_final['사업자번호'].isin(wb_ids)].groupby(['사업자번호', '거래처명', '지역'])['매출액'].sum()
    s_hist = df_raw[df_raw['사업자번호'].isin(wb_ids)].groupby(['사업자번호', '거래처명', '지역'])['매출액'].max()
    df_wb = pd.DataFrame(index=s_curr.index); df_wb['올해매출'] = s_curr; df_wb['과거최고'] = s_hist
    ...
    with col_ch:
        fig = px.scatter(df_wb, x='과거최고', y='올해매출', color='상태', hover_name='거래처명', size='Bubble_Size', color_discrete_map={"완전 회복": "green", "회복 중": "orange", "재진입 초기": "red"})
...
if is_edit_mode:
    with st.sidebar:
        ...
        if st.button("🔗 축약 공유 링크 생성"):
            ...
            p_string = f"?y={'&y='.join(map(str, sel_years))}&c={sel_channels[0]}&q={'&q='.join(map(str, sel_quarters))}&m={'&m='.join(map(str, sel_months))}&cat={cat_p}&prod={prod_p}"
            st.success("링크 생성 완료!"); st.code(base_url + p_string)
...
with tab5:
    ...
    if not p_v.empty:
        st.subheader("📊 데이터 요약")
        st.write(f"• 최다 판매 제품: **{p_v.iloc[0]['제품명']}** / 최대 매출 제품군: **{cat_v.iloc[0]['제품군']}**")
    
    st.markdown("### 📦 제품별 판매 현황")
    # [요청사항 반영] 그래프 5:5 배치 섹션을 제목 밑으로 이동
    g1, g2 = st.columns(2)
